  ...
  template: `
    @if (oc()) {
      <div class="max-w-5xl mx-auto p-6 space-y-6">
        <!-- HEADER & ACTIONS -->
        <div
          class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl border border-gray-200 shadow-sm"
        >
          <div>
            <div class="flex items-center gap-3 mb-1">
              <h1 class="text-2xl font-black text-gray-800">{{ oc()?.nroOC }}</h1>
              <span
                class="px-2.5 py-1 text-[10px] font-black rounded-full uppercase bg-gray-100 text-gray-800"
                >{{ oc()?.estado?.replace('_', ' ') }}</span
              >
            </div>
            <p class="text-sm text-gray-500 font-medium">
              {{ oc()?.proveedorNombre }} - Monto:
              <span class="text-gray-900 font-bold">{{ oc()?.montoTotal | currency: 'USD' }}</span>
            </p>
          </div>
          ...
          <div class="flex flex-wrap gap-2">
            ...
            @if (oc()?.estado === OrdenCompraEstado.APROBADA && rol() === RolSimulado.DIR_COMPRAS) {
              <button
                (click)="enviar()"
                class="px-4 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-sm text-sm"
              >
                Enviar al Proveedor
              </button>
            }
            ...
            <button
              routerLink="/compras"
              class="px-4 py-2 bg-white text-gray-700 font-bold rounded-lg border border-gray-300 hover:bg-gray-50 shadow-sm text-sm"
            >
              Volver
            </button>
          </div>
        ...
        <!-- TIMELINE / CHAT (1.d) -->
        <div
          class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col h-[500px]"
        >
          <div
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
            @for (msg of oc()?.historial; track msg) {
              ...
              >
                <span class="text-[10px] font-bold text-gray-400 px-1"
                ...
                @if (msg.isAdjunto) {
                  <div
                    class="px-4 py-3 rounded-2xl max-w-[80%] shadow-sm border border-gray-200 flex items-center gap-3 bg-white"
          ...
          <div class="p-3 bg-white border-t border-gray-200">
            <div class="flex gap-2 relative">
              ...
              <input
                ...
                class="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
              />
              <button
                (click)="enviarMensaje()"
                [disabled]="!nuevoMensaje.trim()"
                class="bg-blue-600 text-white px-4 rounded-xl font-bold hover:bg-blue-700 disabled:bg-gray-300 transition-colors"
              >
                Enviar
              </button>
            </div>
